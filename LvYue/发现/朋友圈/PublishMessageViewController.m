...
- (void)sendClick{
    ...
    [manager POST:urlStr parameters:@{} success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ( [[NSString stringWithFormat:@"%@",responseObject[@"code"]] isEqualToString:@"200"]){
            ...
            //3.向后台请求
            [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/notice/publish",REQUESTHEADER] andParameter:@{@"publisher":[NSString stringWithFormat:@"%@",[LYUserService sharedInstance].userID],@"noticeType":@"0",@"noticeDetail":_textView.text,@"photos":photoString} success:^(id successResponse) {
