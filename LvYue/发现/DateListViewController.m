...
- (void)getDataFromWeb{
    ...
    [[LYUserService sharedInstance] fetchLoginStateWithCompeletionBlock:^(UserLoginStateType type) {
        if (type == UserLoginStateTypeWaitToLogin) {
            [MBProgressHUD showMessage:nil toView:self.view];
            [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/dating/list",REQUESTHEADER] andParameter:@{@"type":self.dateType,@"latitude":self.latitude,@"longitude":self.longitude,@"isToristEnter":@"1"} success:^(id successResponse) {
            ...
            [MBProgressHUD showMessage:nil toView:self.view];
            [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/dating/list",REQUESTHEADER] andParameter:@{@"user_id":[LYUserService sharedInstance].userID,@"type":self.dateType,@"latitude":self.latitude,@"longitude":self.longitude} success:^(id successResponse) {
                MLOG(@"结果:%@",successResponse);
                [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                if ([[NSString stringWithFormat:@"%@",successResponse[@"code"]] isEqualToString:@"200"]) {
                    [self.modelArray removeAllObjects];
...
- (void)applyBtnClick:(UIButton *)sender{
    [[LYUserService sharedInstance] fetchLoginStateWithCompeletionBlock:^(UserLoginStateType type) {
        if (type == UserLoginStateTypeWaitToLogin) {
            ...
            [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/group/enterRequest",REQUESTHEADER] andParameter:@{@"user_id":[NSString stringWithFormat:@"%@",[LYUserService sharedInstance].userID],@"group_id":self.dateModel.group_id,@"type":@"123"} success:^(id successResponse) {
                MLOG(@"结果:%@",successResponse);
                if ([[NSString stringWithFormat:@"%@",successResponse[@"code"]] isEqualToString:@"200"]) {
