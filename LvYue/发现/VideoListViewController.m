...
- (void)loadVideosWithType:(NSString *)categoryID showMB:(BOOL)isShowMB {
    ...
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/auth_video_list",REQUESTHEADER] andParameter:@{@"pageNum":[NSString stringWithFormat:@"%ld",(long)_currentPage],} success:^(id successResponse) {
        if ([[NSString stringWithFormat:@"%@",successResponse[@"code"]] isEqualToString:@"200"]) {
            MLOG(@"视频列表 : %@",successResponse);
            if (isShowMB) {
                [MBProgressHUD hideHUDForView:self.view animated:YES];
...
- (void)praiseOperation:(UIButton *)sender {
    ...
    if ([model.isPraiseByMe isEqualToString:@"1"]) {
        //取消赞
        [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/video/cancelPraise",REQUESTHEADER] andParameter:@{@"videoId":model.videoID,@"praiseId":[LYUserService sharedInstance].userID} success:^(id successResponse) {
