...
- (void)praiseOperation:(UIButton *)sender {
    ...
    if ([model.isPraiseByMe isEqualToString:@"1"]) {
        //取消赞
        [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/video/cancelPraise", REQUESTHEADER] andParameter:@{ @"videoId": model.videoID,
                                                                                                                                       @"praiseId": [LYUserService sharedInstance].userID }
            success:^(id successResponse) {
