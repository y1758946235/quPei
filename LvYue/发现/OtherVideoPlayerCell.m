...
@interface OtherVideoPlayerCell ()
...
@implementation OtherVideoPlayerCell
...
- (void)awakeFromNib {
    ...
    self.deleteBtn.layer.borderColor = RGBACOLOR(29, 189, 159, 1.0).CGColor;
    self.deleteBtn.layer.borderWidth = 1.0;
}


+ (instancetype)otherVideoPlayerCellWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath {
    static NSString *reuseID = @"otherVideoCell";
    OtherVideoPlayerCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseID];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"OtherVideoPlayerCell" owner:nil options:nil] lastObject];
        cell.layer.cornerRadius = 10.0;
        cell.layer.masksToBounds = YES;
        cell.previewImageView.contentMode = UIViewContentModeScaleAspectFit;
        cell.layer.masksToBounds = YES;
        cell.iconView.userInteractionEnabled = YES;
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:cell action:@selector(tapUserDetail:)];
...
- (void)fillDataWithModel:(VideoDetail *)model {
    ...
    self.praiseNumLabel.text = model.praiseNum;
...
- (void)tapUserDetail:(UITapGestureRecognizer *)gesture {
    //通知控制器
    [[NSNotificationCenter defaultCenter] postNotificationName:@"VideoCircle_TapUserIcon_Other" object:nil userInfo:@{@"indexPath":self.indexPath}];
}
