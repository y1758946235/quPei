...
@interface otherDataVC ()<UITableViewDelegate,UITableViewDataSource,CLLocationManagerDelegate,BMKGeoCodeSearchDelegate>{
...
-(void)getAppointData{
    ...
    NSDictionary *dic = @{@"userId":self.otherUserId,@"otherUserId":[CommonTool getUserID],@"pageNum":[NSString stringWithFormat:@"%ld",(long)currentPage2]};
...
//    [cell.changeBtn removeFromSuperview];
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
//    if ([self.otherZhuYeVC isEqualToString:@"otherZhuYeVC"]) {
//        myDataModel *model = self.dateArr[indexPath.row];
//        [self inviteMyAppoint:model.dataId];
//    }
...
- (void)setNav{
    ...
    self.title = [NSString stringWithFormat:@"%@发布的约会",self.userName];
    //导航栏title的颜色
