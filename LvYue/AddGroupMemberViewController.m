...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.title = @"添加群成员";
    _finishBtn = [self setRightButton:nil title:@"邀请" target:self action:@selector(finishCreate:) rect:CGRectMake(0, 0, 70, 44)];
    ...
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/userFriend/list",REQUESTHEADER] andParameter:@{@"user_id":[LYUserService sharedInstance].userID} success:^(id successResponse) {
        if ([[NSString stringWithFormat:@"%@",successResponse[@"code"]] isEqualToString:@"200"]) {
            ...
            for (NSDictionary *dict in list) {
                ...
                BOOL isExist = NO;
                for (NSString *userID in _alreadMembers) {
                    if ([userID isEqualToString:model.buddyID]) {
                        isExist = YES;
                    }
                }
                if (!isExist) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell.selectView.image) {
        ...
        if (_selectCellArray.count == 0) {
            [_finishBtn setTitle:[NSString stringWithFormat:@"邀请"] forState:UIControlStateNormal];
            _finishBtn.enabled = NO;
        } else {
            [_finishBtn setTitle:[NSString stringWithFormat:@"邀请(%lu)",(unsigned long)_selectCellArray.count] forState:UIControlStateNormal];
...
- (void)finishCreate:(UIButton *)sender {
    ...
    MLOG(@"得到的邀请人id串:%@",finalString);
    
    //继续发送邀请成员请求
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/group/inviteRequest",REQUESTHEADER] andParameter:@{@"group_id":_groupID,@"user_id":[LYUserService sharedInstance].userID,@"other_user_id":finalString} success:^(id successResponse) {
        if ([[NSString stringWithFormat:@"%@",successResponse[@"code"]] isEqualToString:@"200"]) {
            [MBProgressHUD hideHUD];
