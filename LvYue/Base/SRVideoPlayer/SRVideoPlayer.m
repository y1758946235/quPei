...
#pragma mark - Monitor Methods 不让界面旋转
...
//    [_player removeTimeObserver:_playbackTimeObserver];
    ...
    [_playerView.superview removeFromSuperview];
}
...
- (void)showTopBottomBar {
    ...
    if (_playerState != SRVideoPlayerStatePlaying) {
       ...
       [[NSNotificationCenter defaultCenter] postNotificationName:@"sendMessAndPraViewHide" object:nil userInfo:@{@"isHide":@"1"}];
    self.topBar.hidden = NO;
    self.bottomBar.hidden = NO;
    [self timingHideBottomBarTime];
}
...
- (void)hideTopBottomBar {
    ...
    self.topBar.hidden = YES;
    self.bottomBar.hidden = YES;
     [[NSNotificationCenter defaultCenter] postNotificationName:@"sendMessAndPraViewHide" object:nil userInfo:@{@"isHide":@"0"}];
}
