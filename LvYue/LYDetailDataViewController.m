...
+ (void)configTableViewDataArray:(MyDetailInfoModel *)detailInfoModel
    ...
    LYDetailDataTableViewDataArray = @[
        @[
           @{ @"title": @"个性签名",
              @"value": infoModel && infoModel.signature.length ? infoModel.signature : @"暂无信息",
              @"rowHeight": @44,
              @"actionVC": @"" },
           @{ @"title": @"备注",
              ...
              @"actionVC": @"" },
           @{ @"title": @"地区",
              @"value": detailInfoModel && detailInfoModel.city.length ? detailInfoModel.city : @"暂无信息",
              @"rowHeight": @44,
              @"actionVC": @"" },
           @{ @"title": @"个人动态",
              @"value": dynamicImageURLArray && dynamicImageURLArray ? dynamicImageURLArray : [NSNull null],
              @"rowHeight": @82,
              @"actionVC": @"" },
           @{ @"title": @"TA的气质",
              @"value": taDeQiZhiImageURLArray && taDeQiZhiImageURLArray ? taDeQiZhiImageURLArray : [NSNull null],
              @"rowHeight": @82,
              @"actionVC": @"" },
           @{ @"title": @"精华相册",
              @"value": jingHuaImageURLArray && jingHuaImageURLArray ? jingHuaImageURLArray : @"",
              @"rowHeight": @82,
              @"actionVC": [NSNull null] },
        ],
        @[
           @{ @"title": @"学历",
              @"value": infoModel && infoModel.edu.length ? infoModel.edu : @"暂无信息",
              @"rowHeight": @44,
              @"actionVC": @"" },
           @{ @"title": @"行业",
              @"value": detailInfoModel && detailInfoModel.industry.length ? detailInfoModel.industry : @"暂无信息",
              @"rowHeight": @44,
              @"actionVC": @"" },
           @{ @"title": @"联系方式",
              @"value": detailInfoModel && detailInfoModel.contact.length ? detailInfoModel.contact : @"暂无信息",
              @"rowHeight": @44,
              @"actionVC": @"" }
        ],
        @[
...
+ (void)initialize {
    [LYDetailDataViewController configTableViewDataArray:nil infoModel:nil remark:nil dynamicImageURLArray:nil taDeQiZhiImageURLArray:nil jingHuaImageURLArray:nil];
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSNumber *rowHeight = LYDetailDataTableViewDataArray[indexPath.section][indexPath.row][@"rowHeight"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 3 || indexPath.row == 4 || indexPath.row == 5) {
            ...
            [cell configPhotoArray:LYDetailDataTableViewDataArray[indexPath.section][indexPath.row][@"value"] title:LYDetailDataTableViewDataArray[indexPath.section][indexPath.row][@"title"] essenceImage:essenceImage];
            return cell;
        }
    }


    LYDetailDataDefalutTableViewCell *cell = [[[NSBundle mainBundle] loadNibNamed:@"LYDetailDataDefalutTableViewCell" owner:self options:nil] objectAtIndex:0];
    ...
    if (indexPath.section == 2 && indexPath.row == 1) {
        [cell showSwitchWithOn:self.shield valueChanged:^(UISwitch *sender) {
            [self p_changShield];
        }];
    }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 0) {
        ...
        [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/userFriend/iosDelete", REQUESTHEADER]
            andParameter:@{ @"user_id": [LYUserService sharedInstance].userID,
                            @"friend_user_id": self.userId }
            success:^(id successResponse) {
                if ([[NSString stringWithFormat:@"%@", successResponse[@"code"]] isEqualToString:@"200"]) {
                    //调用环信"删除好友"的方法
                    EMError *error = nil;
                    BOOL isSuccess = [[EaseMob sharedInstance].chatManager removeBuddy:self.userId removeFromRemote:YES error:&error];
                    if (isSuccess && !error) {
                        [MBProgressHUD hideHUD];
                        //删除之前的用户对话
                        [[EaseMob sharedInstance].chatManager removeConversationByChatter:self.userId deleteMessages:YES append2Chat:YES];
                        //发送更新会话列表的通知
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        [self.navigationController pushViewController:[LYSendGiftViewController new] animated:YES];
    }
}
...
- (void)p_loadData {
    ...
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/userFriend/getInfo", REQUESTHEADER]
        andParameter:@{ @"friend_user_id": self.userId,
        ...
        success:^(id successResponse) {
            ...
            if ([[NSString stringWithFormat:@"%@", successResponse[@"code"]] isEqualToString:@"200"]) {
                ...
                NSMutableDictionary *infoDic = [successResponse[@"data"][@"user"] mutableCopy];
                // 关注 粉丝   接口传的结构跟用户信息并列的 为了方便插入到 infoModel
                [infoDic setObject:successResponse[@"data"][@"isFocus"] forKey:@"isFocus"];
                [infoDic setObject:successResponse[@"data"][@"fansNum"] forKey:@"fansNum"];
                self.infoModel       = [[MyInfoModel alloc] initWithDict:[infoDic copy]];
                self.detailInfoModel = [[MyDetailInfoModel alloc] initWithDict:successResponse[@"data"][@"userDetail"]];
                ...
                [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/imgList", REQUESTHEADER] andParameter:@{ @"user_id": self.userId } success:^(id successResponse) {
                    MLOG(@"结果:%@", successResponse);
                    if ([[NSString stringWithFormat:@"%@", successResponse[@"code"]] isEqualToString:@"200"]) {

                        NSMutableArray *array = [[NSMutableArray alloc] init];
                        [successResponse[@"data"][@"list"] enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
                            [array addObject:obj[@"img_name"]];
                        }];
                        self.taDeQiZhiImageURLArray = [array copy];

                        [LYDetailDataViewController configTableViewDataArray:self.detailInfoModel infoModel:self.infoModel remark:self.remark dynamicImageURLArray:self.dynamicImageURLArray taDeQiZhiImageURLArray:self.taDeQiZhiImageURLArray jingHuaImageURLArray:self.taDeQiZhiImageURLArray];
                        [self.tableView reloadData];
...
- (void)p_processRelationShip:(LYDetailDataRelationShipEnum)flag {
    switch (flag) {
        ...
        case LYDetailDataRelationShipEnumIng: {
            ...
            [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/messageVerify/updateIosStatus", REQUESTHEADER]
                andParameter:@{
                    @"user_id": [LYUserService sharedInstance].userID,
                ...
                success:^(id successResponse) {
                    if ([[NSString stringWithFormat:@"%@", successResponse[@"code"]] isEqualToString:@"200"]) {
                        //手动调用环信的"通过验证"方法
                        EMError *error = nil;
                        BOOL isSuccess = [[EaseMob sharedInstance].chatManager acceptBuddyRequest:self.userId error:&error];
                        if (isSuccess && !error) {
                            [MBProgressHUD hideHUD];
                            [MBProgressHUD showSuccess:@"通过验证"];
                            //                            [self getDataFromWeb];
                            //提示通过了XX用户的验证信息(同意加好友)
                            EMConversation *conversation = [[EaseMob sharedInstance].chatManager conversationForChatter:[NSString stringWithFormat:@"%ld", (long) self.userId] conversationType:eConversationTypeChat];
    ...
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/video/openVideo", REQUESTHEADER] andParameter:@{
        ...
        success:^(id successResponse) {
            ...
            if ([[NSString stringWithFormat:@"%@", successResponse[@"code"]] isEqualToString:@"200"]) {
                ...
                MPMoviePlayerViewController *player = [[MPMoviePlayerViewController alloc] initWithContentURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", IMAGEHEADER, self.detailInfoModel.authVideoPath]]];
...
- (void)p_changShield {
    ...
    // 屏蔽
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/userFriend/deleteShield", REQUESTHEADER]
        andParameter:@{
 ...
 */
- (void)p_foucsOn {
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/userFriend/focusOther", REQUESTHEADER] andParameter:@{
        @"user_id": [LYUserService sharedInstance].userID,
...
- (void)p_clickInviteVedioAuthButton:(id)sender {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"豆客" message:@"TA 还未通过形象视频认证，请谨慎联系。确认送礼并邀请 TA 认证形象视频吗？如果对方不认证，系统会返回您购买该礼物的金币" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    [alert show];
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        _tableView = ({
            ...
            //            [tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:LYDetailDataTableViewDefaultCellIdentity];
...
- (UIView *)detailDataHeaderView {
    if (!_detailDataHeaderView) {
        _detailDataHeaderView                    = [[[NSBundle mainBundle] loadNibNamed:@"LYDetailDataHeaderView" owner:self options:nil] objectAtIndex:0];
...
- (UIView *)invitateTableViewHeaderView {
    if (!_invitateTableViewHeaderView) {
        _invitateTableViewHeaderView = ({
            ...
            [label sizeToFit];
            label.frame = CGRectMake(14.f, (30.f - label.height) / 2.f, label.width, label.height);
            [view addSubview:label];

            UIButton *button = [[UIButton alloc] init];
            [button setTitle:@"点击邀请认证" forState:UIControlStateNormal];
            [button setTitleColor:RGBCOLOR(17, 198, 173) forState:UIControlStateNormal];
            [button.titleLabel setFont:[UIFont systemFontOfSize:14.f]];
            [button addTarget:self action:@selector(p_clickInviteVedioAuthButton:) forControlEvents:UIControlEventTouchUpInside];
            [button sizeToFit];
            button.frame = CGRectMake(CGRectGetMaxX(label.frame) + 5.f, (30.f - button.height) / 2.f, button.width, button.height);
...
- (UIView *)tableViewFooterView {
    if (!_tableViewFooterView) {
        _tableViewFooterView = ({
            UIView *view                = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 175.f)];
            UIButton *sendMessageButton = [[UIButton alloc] initWithFrame:CGRectMake(20.f, 40.f, SCREEN_WIDTH - 20.f * 2.f, 44.f)];
            ...
            [sendMessageButton addTarget:self action:@selector(p_sendMessage:) forControlEvents:UIControlEventTouchUpInside];

            UIButton *addFriendButton = [[UIButton alloc] initWithFrame:CGRectMake(20.f, CGRectGetMaxY(sendMessageButton.frame) + 16.f, SCREEN_WIDTH - 20.f * 2.f, 44.f)];
            ...
            switch (self.status) {
                ...
                case LYDetailDataRelationShipEnumDelete:
                    ...
                    break;
                case LYDetailDataRelationShipEnumIng:
                    [addFriendButton setTitle:@"已经发送申请加为好友" forState:UIControlStateNormal];
                    break;
            }

            [addFriendButton setTitleColor:RGBCOLOR(17, 198, 173) forState:UIControlStateNormal];
            addFriendButton.backgroundColor = [UIColor whiteColor];
            [addFriendButton addTarget:self action:@selector(p_addFriend:) forControlEvents:UIControlEventTouchUpInside];
