    ...
    [manager POST:urlStr parameters:@{} success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ( [[NSString stringWithFormat:@"%@",responseObject[@"code"]] isEqualToString:@"200"]){
            ...
            for (int i = 0; i < _imgArray.count; i++) {
                ...
                //3.向后台请求
                [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/addUserImg",REQUESTHEADER] andParameter:@{@"user_id":[NSString stringWithFormat:@"%@",[LYUserService sharedInstance].userID],@"img_name":photoStr,@"intro":markView.text} success:^(id successResponse) {
