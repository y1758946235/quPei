...
@interface moneyListVc ()<UITableViewDelegate,UITableViewDataSource>{
    UITableView *table;
    NSMutableArray *moneyListArr;
...
@implementation moneyListVc

- (void)viewDidLoad {
    [super viewDidLoad];
    moneyListArr = [[NSMutableArray alloc]init];
    [self setTable];
    [self setNav];
    currentPage2 = 1;
    [self getData];
...
- (void)getData{
    ...
    [LYHttpPoster requestBuyMoneyWithParameters:dic block:^(NSMutableArray *arr) {
        [moneyListArr removeAllObjects];
        [moneyListArr addObjectsFromArray:arr];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return moneyListArr.count;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
   moneyListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (!cell) {
        cell = [[moneyListCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
    }
    if (moneyListArr.count >0) {
      
        MoneyListModel *model = moneyListArr[indexPath.row];
        cell.model = model;
    }
