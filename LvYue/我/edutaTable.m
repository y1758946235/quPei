...
@interface edutaTable ()<UITableViewDelegate,UITableViewDataSource>{
...
@implementation edutaTable

- (void)viewDidLoad {
    [super viewDidLoad];
    ageArr = @[@"博士及以上",@"硕士",@"本科",@"大专",@"高中",@"初中",@"小学"];
...
- (void)save{
    if ([CommonTool dx_isNullOrNilWithObject:eduStr]) {
        [MBProgressHUD showError:@"请选择学历"];
        return;
    }
      NSString *userId = [[NSUserDefaults standardUserDefaults]objectForKey:@"userId"];
    
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/updatePersonalInfo",REQUESTHEADER] andParameter:@{@"userId":userId,@"userQualification":eduStr} success:^(id successResponse) {
