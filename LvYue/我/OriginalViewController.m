...
- (void)longPress:(UILongPressGestureRecognizer *)longPress {
    if (!actionSheet) {
        if ([[NSString stringWithFormat:@"%@",_userId] isEqualToString:[CommonTool getUserID]]) {
            actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存到相册",@"删除", nil];
        } else {
...
//    if ( [[NSString stringWithFormat:@"%@",self.userId] isEqualToString:[CommonTool getUserID]]) {
        [self addPageControllerWithIndex:_index andCount:count];
...
- (void)loadImageWithIndex:(NSInteger)index {
    ...
    if ( [[NSString stringWithFormat:@"%@",self.userId] isEqualToString:[CommonTool getUserID]]) {
        ...
        NSDictionary *dic                     = self.imageData[index];
        if ([dic[@"photoPrice"] doubleValue]> 0 &&  [dic[@"isLook"] integerValue]== 0) {
           
                photoView.effectView.hidden= NO;
            
            
           
        }
    }
       if ([NSURL URLWithString:[NSString stringWithFormat:@"%@%@", IMAGEHEADER, _imageData[index % _imageData.count][@"photoUrl"]]]) {
        __block UIActivityIndicatorView *activityIndicator;

        [photoView.imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", IMAGEHEADER, _imageData[index % _imageData.count][@"photoUrl"]]] placeholderImage:nil options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
//    NSString *createTime     = [_imageData[index % _imageData.count][@"createTime"] isEqual:[NSNull null]] ? @"" : _imageData[index % _imageData.count][@"createTime"];
     NSString *createTime = [NSString stringWithFormat:@"%@",_imageData[index % _imageData.count][@"createTime"]] ;
    if (![CommonTool dx_isNullOrNilWithObject:createTime]) {
       
        photoView.timeLabel.text =[CommonTool timestampSwitchTime:[createTime doubleValue]/1000 andFormatter:@"YYYY-MM-dd"];
    }
    

    if (_imageData[index % _imageData.count][@"photoSignature"] && [_imageData[index % _imageData.count][@"photoSignature"] length] > 0) {
        photoView.markLabel.text = [NSString stringWithFormat:@" %@", _imageData[index % _imageData.count][@"photoSignature"]];
    }
...
//    }else  (buttonIndex == 1 && [_userId isEqualToString:[LYUserService sharedInstance].userID]) {
    }else if(buttonIndex==1){
//       if ([[NSString stringWithFormat:@"%@",_userId] isEqualToString:[LYUserService sharedInstance].userID])
         if ([[NSString stringWithFormat:@"%@",_userId] isEqualToString:[CommonTool getUserID]]){
        [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/deleteUserPhoto", REQUESTHEADER] andParameter:@{@"photoIdArr": [NSString stringWithFormat:@"%@",  _imageData[_index][@"photoId"]] } success:^(id successResponse) {

            if ([[NSString stringWithFormat:@"%@", successResponse[@"code"]] isEqualToString:@"200"]) {
                [MBProgressHUD hideHUD];
                [MBProgressHUD showSuccess:@"删除成功" toView:self.view];
              [[NSNotificationCenter defaultCenter] postNotificationName:@"reloadMyAlbum" object:nil];
