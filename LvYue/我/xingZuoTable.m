...
@interface xingZuoTable ()<UITableViewDelegate,UITableViewDataSource>{
    UITableView *table;
    NSArray *ageArr;
    NSString *costeStr;
}
...
@implementation xingZuoTable

- (void)viewDidLoad {
    [super viewDidLoad];
    ageArr = @[@"水瓶座",@"双鱼座",@"白羊座",@"金牛座",@"双子座",@"巨蟹座",@"狮子座",@"处女座",@"天秤座",@"天蝎座",@"射手座",@"摩羯座"];
...
- (void)save{
    ...
    if ([CommonTool dx_isNullOrNilWithObject:costeStr]) {
        [MBProgressHUD showError:@"请选择星座"];
        return;
    }
      NSString *userId = [[NSUserDefaults standardUserDefaults]objectForKey:@"userId"];
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/updatePersonalInfo",REQUESTHEADER] andParameter:@{@"userId":userId,@"userConstellation":costeStr} success:^(id successResponse) {
