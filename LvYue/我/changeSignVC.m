...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    //实时显示字数
    numLabel.text = [NSString stringWithFormat:@"%lu/40", (unsigned long)textView.text.length];
    ...
    if (textView.text.length >= 40) {
        ...
        numLabel.text = @"40/40";
        [MBProgressHUD showSuccess:@"字数不能超过40字哦～～"];
...
- (void)save{
    ...
    if ([CommonTool dx_isNullOrNilWithObject:_textView.text]) {
        [MBProgressHUD showError:@"请编辑个性签名"];
        return;
    }
      NSString *userId = [[NSUserDefaults standardUserDefaults]objectForKey:@"userId"];
    
    [LYHttpPoster postHttpRequestByPost:[NSString stringWithFormat:@"%@/mobile/user/updatePersonalInfo",REQUESTHEADER] andParameter:@{@"userId":userId,@"userSignature":_textView.text} success:^(id successResponse) {
